...
import { parseSpedPISCOFINSNaoCumulativo } from '@/lib/parsers/parserSpedPISCOFINSNaoCumulativo';

export default function UploadLucroReal() {
  ...
  const createMutation = useMutation({
    mutationFn: (data) => base44.entities.ApuracaoLucroReal.create(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['apuracoes-lr'] });
  ...
  const handleProcessar = async () => {
    ...
    try {
      ...
      if (pisFile) {
        ...
        pisData = parseSpedPISCOFINSNaoCumulativo(text);
      }
      ...
      try {
        const alertasResp = await base44.integrations.Core.InvokeLLM({
          prompt: `Você é um especialista tributário brasileiro. Analise os dados extraídos dos SPEDs de uma empresa do Lucro Real referente ao período ${periodo} e aponte possíveis erros, inconsistências e alertas tributários.

CONTEXTO TRIBUTÁRIO IMPORTANTE:
1. ICMS: O saldo apurado NÃO é simplesmente (Débito - Crédito). O cálculo correto é: (Débitos + Ajustes a Débito) - (Créditos + Ajustes a Crédito + Saldo Credor do Período Anterior). NÃO flague como erro matemático a diferença entre Débito e Crédito sem considerar o saldo credor anterior.
2. PIS e COFINS: No Lucro Real, o regime é NÃO-CUMULATIVO — a empresa apura créditos sobre insumos, energia elétrica, aluguéis, depreciação, frete e outras despesas (registros M100/M500), que são descontados dos débitos apurados sobre a receita (M210/M610, alíquotas de 1,65% e 7,60%). O saldo a recolher é Débito - Crédito. A existência de créditos relevantes é o comportamento ESPERADO. NÃO flague a existência de créditos como erro.
3. Receita de Serviços Prestados: valores extraídos do registro F550 do SPED PIS/COFINS sem CFOP vinculado.
4. Se os créditos de PIS/COFINS parecerem desproporcionalmente altos ou baixos em relação à receita, considere que isso pode refletir a estrutura de custos da empresa (ex: empresas de serviço têm poucos insumos, empresas comerciais/industriais têm mais).
...
- PIS Débito (não-cumulativo): R$ ${(pisData.pis_debito || 0).toFixed(2)}
...
Retorne até 8 alertas objetivos e práticos, classificados como "erro" (inconsistência grave), "aviso" (atenção necessária) ou "info" (informação relevante). Seja específico com os valores quando relevante.`,
      ...
      const existentes = await base44.entities.ApuracaoLucroReal.filter({
        empresa_id: empresaId,
        periodo,
      });
  ...
  const handleConfirmReplace = () => {
    ...
    base44.entities.ApuracaoLucroReal.delete(_existenteId)
  ...
  return (
    <div className="p-6 md:p-8 max-w-4xl mx-auto">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-foreground">Upload de SPEDs — Lucro Real</h1>
        <p className="text-muted-foreground text-sm mt-1">Processamento do SPED EFD ICMS/IPI e EFD Contribuições PIS/COFINS (não-cumulativo)</p>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            />
            <p className="text-xs text-muted-foreground">
              Arquivo EFD Contribuições — contém apuração de créditos e débitos de PIS e COFINS (regime não-cumulativo)
            </p>
          </div>
