...
export default function HistoricoLucroReal() {
  ...
  const { data: allApuracoes = [], isLoading } = useQuery({
    queryKey: ['apuracoes-lr', selectedEmpresaId],
    queryFn: () =>
      selectedEmpresaId
        ? base44.entities.ApuracaoLucroReal.filter({ empresa_id: selectedEmpresaId }, '-periodo', 200)
        : Promise.resolve([]),
    enabled: !!selectedEmpresaId,
  });

  const deleteMutation = useMutation({
    mutationFn: (id) => base44.entities.ApuracaoLucroReal.delete(id),
    onSuccess: () => {
  ...
  return (
    <div className="p-6 md:p-8 max-w-7xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground text-sm mt-1">Lucro Real — Visualize e gerencie as apurações mensais de SPEDs</p>
      </div>
