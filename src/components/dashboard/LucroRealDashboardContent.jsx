...
import { TrendingUp, ShoppingCart, AlertTriangle, Info, AlertCircle, X, Calendar, FileBarChart, Wallet } from 'lucide-react';
          ...
          {showSaldo && (
            <div className={`text-center p-3 rounded-lg border ${isDevedor ? 'bg-amber-50 border-amber-200' : 'bg-green-50 border-green-200'}`}>
              ...
              <div className={`text-sm font-semibold ${isDevedor ? 'text-amber-700' : 'text-green-700'}`}>
                ...
                <span className="block text-xs font-normal">{isDevedor ? 'A Recolher' : 'Credor'}</span>
              </div>
            </div>
          )}
        </div>
...
function HorizontalBar({ items, labelKey, valueKey, colorClass = 'bg-primary', showCfopDesc = false, subKey }) {
  ...
  return (
    <div className="space-y-2">
      {items.map((item, idx) => {
        const descricao = showCfopDesc ? getCfopDescricao(item[labelKey]) : null;
...
// Conteúdo completo do dashboard do Lucro Real. Segue a mesma estrutura visual e
// de navegação dos dashboards de Simples Nacional e Lucro Presumido, com uma seção
// adicional exclusiva: apuração de créditos x débitos de PIS/COFINS (regime não-cumulativo).
export default function LucroRealDashboardContent({
  empresa,
  apuracoes = [],
  isLoading = false,
  emptyActionHref,
  ...
  const agregado = useMemo(() => {
    ...
    return {
      ...
      pis_creditos_por_natureza: mergeByKey(ultimosMeses.map(a => a.pis_creditos_por_natureza), 'natureza', ['valor']),
      cofins_creditos_por_natureza: mergeByKey(ultimosMeses.map(a => a.cofins_creditos_por_natureza), 'natureza', ['valor']),
    };
  }, [ultimosMeses]);
      ...
      {apuracao && (
        <>
          ...
          <div className={`grid grid-cols-2 gap-3 ${apuracao.total_servicos_prestados > 0 ? 'md:grid-cols-6' : 'md:grid-cols-5'}`}>
            ...
            <MetricCard title="PIS" value={fmtBRL(Math.abs(apuracao.pis_saldo))} sub={(apuracao.pis_saldo || 0) >= 0 ? 'A Recolher' : 'Saldo Credor'} variant={(apuracao.pis_saldo || 0) >= 0 ? 'warning' : 'success'} />
            <MetricCard title="COFINS" value={fmtBRL(Math.abs(apuracao.cofins_saldo))} sub={(apuracao.cofins_saldo || 0) >= 0 ? 'A Recolher' : 'Saldo Credor'} variant={(apuracao.cofins_saldo || 0) >= 0 ? 'warning' : 'success'} />
          </div>
          ...
          {/* PIS/COFINS — Créditos e Débitos (Não Cumulativo) — seção exclusiva do Lucro Real */}
          <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Card>
              ...
              <CardContent>
                <HorizontalBar items={apuracao.vendas_por_cfop || []} labelKey="cfop" valueKey="valor" colorClass="bg-primary" showCfopDesc />
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="pb-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Card>
              ...
              <CardContent>
                <HorizontalBar items={apuracao.compras_por_cfop || []} labelKey="cfop" valueKey="valor" colorClass="bg-chart-3" showCfopDesc />
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="pb-2">
