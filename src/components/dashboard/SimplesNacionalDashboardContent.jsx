...
import { TrendingUp, FileBarChart, PieChartIcon, Calendar } from 'lucide-react';
...
import EmptyChartMsg from '@/components/dashboard/EmptyChartMsg';
...
// Conteúdo completo do dashboard do Simples Nacional (filtros de período, cards,
// gráficos e ranking de CFOP). Usado tanto na página autenticada quanto na página
// pública de compartilhamento, para que ambas mostrem exatamente as mesmas seções.
export default function SimplesNacionalDashboardContent({
  empresa,
  apuracoes = [],
  isLoading = false,
  emptyActionHref,
  ...
  const ultimosMeses = useMemo(() => apuracoesOrdenadas.slice(-mesesVisiveis), [apuracoesOrdenadas, mesesVisiveis]);
  ...
  const periodosDisponiveis = useMemo(() => apuracoesOrdenadas.map((a) => a.periodo).reverse(), [apuracoesOrdenadas]);

  const barData = useMemo(() => ultimosMeses.map((a) => ({
  ...
  const impostoData = useMemo(() => ultimosMeses.map((a) => ({
    mes: formatMesAno(a.periodo),
  ...
  return (
    <div className="space-y-6">
      {/* Filtros de período */}
      <div className="flex flex-wrap items-center gap-3" data-no-export>
